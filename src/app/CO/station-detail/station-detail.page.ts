...
export class StationDetailPage implements OnInit {
      ...
      "Do you want to change the status?").then((res: any) => {
        ...
        if (!res) {
          ...
          self.commonServices.post("CommonGetData", { Flag: 'UpdateSOStatus',
           Id: StationId, Status: this.status }).subscribe(
            (res: any) => {
  ...
  getCoStationData() {
    var self = this;
    self.commonServices.loadingPresent();
    self.commonServices.post("CommonGetData", { Flag: 'COListForCR', Id: self.userId }).subscribe((res: any) => {
  ...
  exportFile(){
    this.commonServices.post("getAllDetails_CSV", { Flag: "COListForCR", Id: this.userId, PageFlag:'COStations', Status: this.status }).subscribe(
      (resp: any) => {
